...
const products = ref<Product[]>([
  ...
  const getProductsByCategory = (category: string) => {
    ...
    // Function to save filtered products to local storage
  const saveFilteredProductsToLocalStorage = (category: string) => {
    const filteredProducts = getProductsByCategory(category);
    localStorage.setItem('filteredProducts', JSON.stringify(filteredProducts));
  };

    // Function to load filtered products from local storage
  const loadFilteredProductsFromLocalStorage = () => {
    const filteredProducts = localStorage.getItem('filteredProducts');
    if (filteredProducts) {
      return JSON.parse(filteredProducts) as Product[];
    }
    return [];
  };
  ...
  return { products, getProductById, setProduct, loadProductFromLocalStorage ,getProductsByCategory,saveFilteredProductsToLocalStorage,loadFilteredProductsFromLocalStorage };
